...
void OctSpatialPartition::AddObjectToCell(OctNode* cell_, GameObject* obj_)
{
	if (cell_ != nullptr)
	{
		...
		if (obj_->GetBoundingBox().Intersects(cellBB))
		{
			...
			{
				for (int i = 0; i < 8; i++)
					cell_->children[i]->AddCollisionObject(obj_);
			}
		}
	}
}
...
void OctSpatialPartition::PrepareCollisionQuery(OctNode* cell_, Ray ray_)
{
	if (cell_ != nullptr)
	{
		...
		if (ray_.IsColliding(cellBB))
		{
			...
			{
				for (int i = 0; i < 8; i++)
					rayIntersectionList.push_back(cell_->children[i]);
			}
		}
	}
}